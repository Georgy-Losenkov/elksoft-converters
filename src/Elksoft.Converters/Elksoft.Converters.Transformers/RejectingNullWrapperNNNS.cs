// <copyright file="RejectingNullWrapperNNNS.cs" company="Georgy Losenkov">
...
{
    internal sealed class RejectingNullWrapperNNNS<TIn, TOut> : Converter<TIn?, TOut>
        where TIn : struct
        where TOut : struct
    {
        public RejectingNullWrapperNNNS(Converter<TIn?, TOut?> innerConverter)
        {
        ...
        {
            return value.HasValue ? InnerConverter.Convert(value, formatProvider).GetValueOrDefault() : default;
        }
    }
}