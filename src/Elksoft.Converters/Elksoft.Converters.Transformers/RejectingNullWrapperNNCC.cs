// <copyright file="RejectingNullWrapperNNCC.cs" company="Georgy Losenkov">
...
{
    internal sealed class RejectingNullWrapperNNCC<TIn, TOut> : Converter<TIn?, TOut>
        where TIn : struct
        where TOut : class
    {
        public RejectingNullWrapperNNCC(Converter<TIn?, TOut> innerConverter)
        {
        ...
        {
            return value.HasValue ? InnerConverter.Convert(value, formatProvider) : default;
        }
    }
}