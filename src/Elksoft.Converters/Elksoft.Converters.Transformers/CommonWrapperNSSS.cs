// <copyright file="CommonWrapperNSSS.cs" company="Georgy Losenkov">
...
{
    internal sealed class CommonWrapperNSSS<TIn, TOut> : Converter<TIn?, TOut>
        where TIn : struct
        where TOut : struct
    {
        public CommonWrapperNSSS(Converter<TIn, TOut> innerConverter)
        {
        ...
        {
            return value.HasValue ? InnerConverter.Convert(value.GetValueOrDefault(), formatProvider) : default;
        }
    }
}