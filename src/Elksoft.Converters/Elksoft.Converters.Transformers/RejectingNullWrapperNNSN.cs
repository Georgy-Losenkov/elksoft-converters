// <copyright file="RejectingNullWrapperNNSN.cs" company="Georgy Losenkov">
...
{
    internal sealed class RejectingNullWrapperNNSN<TIn, TOut> : Converter<TIn?, TOut?>
        where TIn : struct
        where TOut : struct
    {
        public RejectingNullWrapperNNSN(Converter<TIn?, TOut> innerConverter)
        {
        ...
        {
            return value.HasValue ? InnerConverter.Convert(value, formatProvider) : default(TOut?);
        }
    }
}