...
{
    public sealed class StringToUIntPtrConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToUIntPtrConverter();
        ...
        {
            var converter = new StringToUIntPtrConverter();
            converter.Convert(input, provider).Should().Be(UIntPtr.Parse(input, provider));
        }
