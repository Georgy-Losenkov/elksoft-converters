...
{
    public sealed class StringToDateTimeOffsetConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToDateTimeOffsetConverter();
        ...
        {
            var converter = new StringToDateTimeOffsetConverter();
            converter.Convert(input, provider).Should().Be(DateTimeOffset.Parse(input, provider));
        }
