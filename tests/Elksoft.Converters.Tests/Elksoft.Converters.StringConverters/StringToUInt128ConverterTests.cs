...
{
    public sealed class StringToUInt128ConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToUInt128Converter();
        ...
        {
            var converter = new StringToUInt128Converter();
            converter.Convert(input, provider).Should().Be(UInt128.Parse(input, provider));
        }
