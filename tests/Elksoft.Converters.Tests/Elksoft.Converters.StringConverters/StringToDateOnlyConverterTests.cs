...
{
    public sealed class StringToDateOnlyConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToDateOnlyConverter();
        ...
        {
            var converter = new StringToDateOnlyConverter();
            converter.Convert(input, provider).Should().Be(DateOnly.Parse(input, provider));
        }
