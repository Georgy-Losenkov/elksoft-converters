...
{
    public sealed class StringToHalfConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToHalfConverter();
        ...
        public static TheoryData<String, IFormatProvider> Convert_NotNullInput_ReturnsExpected_Data()
        {
            ...
            return new TheoryData<String, IFormatProvider>() {
                { (-123_456_789_012_345).ToString(formatProvider0), formatProvider0 },
                { (-123_456_789_012_345).ToString(formatProvider1), formatProvider1 },
                { (-123_456_789_012_345).ToString(formatProvider2), formatProvider2 },
                { 123_456_789_012_345.ToString(formatProvider0), formatProvider0 },
                { 123_456_789_012_345.ToString(formatProvider1), formatProvider1 },
                { 123_456_789_012_345.ToString(formatProvider2), formatProvider2 },
            };
        ...
        {
            var converter = new StringToHalfConverter();
            converter.Convert(input, provider).Should().Be(Half.Parse(input, provider));
        }
