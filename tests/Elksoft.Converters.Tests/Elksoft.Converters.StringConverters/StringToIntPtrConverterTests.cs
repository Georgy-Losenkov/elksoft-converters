...
{
    public sealed class StringToIntPtrConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToIntPtrConverter();
        ...
        {
            var converter = new StringToIntPtrConverter();
            converter.Convert(input, provider).Should().Be(IntPtr.Parse(input, provider));
        }
