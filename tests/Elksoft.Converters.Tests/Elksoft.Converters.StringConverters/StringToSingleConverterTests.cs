...
{
    public sealed class StringToSingleConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToSingleConverter();
        ...
        {
            var converter = new StringToSingleConverter();
            converter.Convert(input, provider).Should().Be(Single.Parse(input, provider));
        }
