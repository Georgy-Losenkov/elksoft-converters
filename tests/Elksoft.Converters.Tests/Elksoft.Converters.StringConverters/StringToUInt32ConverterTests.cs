...
{
    public sealed class StringToUInt32ConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToUInt32Converter();
        ...
        {
            var converter = new StringToUInt32Converter();
            converter.Convert(input, provider).Should().Be(UInt32.Parse(input, provider));
        }
