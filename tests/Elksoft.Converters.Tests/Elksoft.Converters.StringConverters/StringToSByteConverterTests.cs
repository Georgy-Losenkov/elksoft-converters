...
{
    public sealed class StringToSByteConverterTests
    {
        [Fact]
        public static void AcceptsNull_ReturnsFalse()
        {
            var converter = new StringToSByteConverter();
        ...
        public static TheoryData<String, IFormatProvider> Convert_NotNullInput_ReturnsExpected_Data()
        {
            ...
            return new TheoryData<String, IFormatProvider>() {
                { (-125).ToString(formatProvider0), formatProvider0 },
                { (-125).ToString(formatProvider1), formatProvider1 },
                { (-125).ToString(formatProvider2), formatProvider2 },
                { 125.ToString(formatProvider0), formatProvider0 },
                { 125.ToString(formatProvider1), formatProvider1 },
                { 125.ToString(formatProvider2), formatProvider2 },
            };
        ...
        {
            var converter = new StringToSByteConverter();
            converter.Convert(input, provider).Should().Be(SByte.Parse(input, provider));
        }
