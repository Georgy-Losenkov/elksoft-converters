...
using WrapperClass = Elksoft.Converters.Transformers.AcceptingNullWrapperNNNS<System.Double, System.Double>;

namespace Elksoft.Converters.Transformers
{
    public class AcceptingNullWrapperNNNSTests
    {
        [Fact]
        ...
        private void Verify_Convert_InvokeOnNullInput_ReturnsExpected(Double? output, IFormatProvider formatProvider)
        {
            ...
            var instance = new WrapperClass(mockInnerConverter.Object);
            instance.Convert(null, formatProvider).Should().Be(output.GetValueOrDefault());
        ...
        private void Verify_Convert_InvokeOnNotNullInput_ReturnsExpected(IFormatProvider formatProvider)
        {
            ...
            var instance = new WrapperClass(mockInnerConverter.Object);
            instance.Convert(input1, formatProvider).Should().Be(output1.GetValueOrDefault());
            instance.Convert(input2, formatProvider).Should().Be(output2.GetValueOrDefault());
            instance.Convert(input3, formatProvider).Should().Be(output3.GetValueOrDefault());
            instance.Convert(input4, formatProvider).Should().Be(output4.GetValueOrDefault());
